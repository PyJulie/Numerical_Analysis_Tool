...
def IP(n,x,yi,Legendra):  # Inner product
    ans = 0
    for i in range(len(yi)):
        ans += yi[i] * P(x[i], n,Legendra)
    return ans


def norm(n,yi,Legendra,x):
    ans = 0
    for i in range(len(yi)):
        ans += P(x[i], n,Legendra) * P(x[i], n,Legendra)
    return ans
...
def Sstar(x,aStar,Legendra):
    ans = 0
    n = len(aStar)
    for i in range(n):
        ans += aStar[i] * P(x, i,Legendra)
    return ans
